...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  // --- Logic for Selecting from Sidebar ---
  const handleSelectHistory = async (item: any) => {
    setResult(item);
    // Fetch the actual file from IndexedDB
    const file = await getFileFromDB(item.id);
    if (file) {
      setSelectedFile(file);
    } else {
      setToast({ message: "Media file not found in storage", type: "error" });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setToast({ message: "Media classification deleted", type: "info" });
  };
  ...
  const handleClearAll = async () => {
    ...
    setToast({ message: "All media classifications deleted", type: "info" });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isDuplicate) {
        ...
        setToast({
          message: "Already analyzed! Showing previous result",
          type: "info",
        });
        setIsLoading(false);
        return;
      ...
      setToast({ message: "Success! Genre analyzed", type: "success" });
    } catch (err: any) {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row min-h-screen bg-transparent transition-colors duration-500">
      ...
      <button
        ...
        className="lg:hidden fixed bottom-6 right-6 z-50 p-4 bg-violet-600 text-white rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all"
      >
        <svg
      ...
      <HistorySidebar
        history={history}
        activeId={result?.id}
        onSelect={handleSelectHistory} // Uses the new fetch-from-DB logic
        ...
        <div className="bg-white dark:bg-[#121212] p-6 lg:p-10 rounded-3xl shadow-2xl w-full max-w-4xl mx-auto border border-gray-100 dark:border-gray-800 transition-all">
          <header className="text-center mb-10">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
              Upload audio or video for AI analysis
            </p>
          </header>
          ...
          <div className="group relative border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl p-6 lg:p-12 text-center hover:border-violet-500 bg-gray-50/50 dark:bg-gray-900/10 transition-all">
            ...
            <div className="flex flex-col items-center gap-3 pointer-events-none">
              <div className="p-4 bg-violet-100 dark:bg-violet-900/30 rounded-full text-violet-600 dark:text-violet-400 group-hover:scale-110 transition-transform">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="32"
                  height="32"
            ...
            <section className="mt-8 p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-gray-100 dark:border-gray-800">
              {selectedFile.type.startsWith("video/") ? (
                <video
                  src={previewUrl}
                  controls
                  className="w-full rounded-xl overflow-hidden max-h-80 bg-black"
                />
              ) : (
          ...
          <footer className="mt-10">
            <button
              ...
              className="w-full py-5 bg-violet-600 hover:bg-violet-700 text-white font-black text-lg rounded-2xl disabled:opacity-50 shadow-xl active:scale-[0.98] transition-all"
            >
